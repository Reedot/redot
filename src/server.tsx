...
app.get('*', (req, res) => {
  ...
  const webExractor = new ChunkExtractor({ statsFile: webStats })

  const store = createStore(reducers)
  const context = {}

  const jsx = webExractor.collectChunks(
  ...
  res.send(`
    ...
    <html lang="en">
      <head>
        ...
        ${webExractor.getLinkTags()}
        ${webExractor.getStyleTags()}
      </head>
      <body>
        <div id="root">${html}</div>
        ${webExractor.getScriptTags()}
      </body>
    </html>
  `)
})
